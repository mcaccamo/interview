...
# when an edge to a visited vertex is popped from the head it is simply skipped. Effectively this means that the heap operates in the same way
# as if the heap did support a priority update for already inserted nodes. The trade-off here is a simpler implementation for a slightly larger memory 
# footprint. LazyDijkstra uses O(|E|) space for the heap, TextBook Dijkstra would only use O(|V|). 
# 
